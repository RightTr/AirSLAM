    ...
    public:
        ...
        size_t size(const std::string& tensorName) const
        {
            for (int i = 0; i < mEngine->getNbIOTensors(); i++)
            {
                const char* name = mEngine->getIOTensorName(i);
                if (tensorName == name)
                    return mManagedBuffers[i]->hostBuffer.nbBytes();
            }
    ...
    private:
        void* getBuffer(const bool isHost, const std::string& tensorName) const
        {
            for (int i = 0; i < mEngine->getNbIOTensors(); i++)
            {
                const char* name = mEngine->getIOTensorName(i);
                if (tensorName == name)
                    return (isHost ? mManagedBuffers[i]->hostBuffer.data() 
        ...
        void memcpyBuffers(const bool copyInput, const bool deviceToHost, const bool async, const cudaStream_t& stream = 0)
        {
            for (int i = 0; i < mEngine->getNbIOTensors(); i++)
            {
                ...
                auto mode = mEngine->getTensorIOMode(tensorName);
                if ((copyInput && mode == nvinfer1::TensorIOMode::kINPUT) || (!copyInput && mode == nvinfer1::TensorIOMode::kOUTPUT))
                {
                    if (async)
