...
```bash
mkdir -p air_ws/src && cd air_ws/src

git clone https://github.com/RightTr/AirSLAM.git
cd AirSLAM
git fetch -a
...
```bash
cd AirSLAM/python
python onnx_node_motify.py

cd ../output
mv superpoint_v1_sim_int32.onnx superpoint_v1_sim_int32_backup.onnx
mv superpoint_v1_fixed.onnx superpoint_v1_sim_int32.onnx
